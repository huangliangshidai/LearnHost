...
if __name__ == "__main__":
    # 使用原始字符串表示路径
    model_path = r'E:/桌面/声学场景分类/生成代码/prehander/model_custom4.h5'
    ...
    for i, segment in enumerate(segments):
        ...
        predictions.append(result)  # 添加此行
        # 删除临时保存的音频文件
        os.remove(segment_file)

    # 在这里添加上述的输出代码
    for i, segment_result in enumerate(segment_results):
        class_label = class_labels.get(segment_result, "Unknown")
        print(f'Segment {i+1} belongs to class: {class_label}')
        print(f'Class probabilities: {segment_probs[i]}')

    # 可以在这里使用整体的predictions进行进一步的分析或处理
    print("Complete predictions:", predictions)
