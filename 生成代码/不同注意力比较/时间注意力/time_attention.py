...
class TemporalAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.b = self.add_weight(shape=(input_shape[-2],), initializer='zeros', trainable=True)
        super(TemporalAttention, self).build(input_shape)

    def call(self, x):
        # 计算时间注意力权重
        temporal_weights = K.softmax(K.dot(x, self.W) + self.b, axis=-2)

        # 应用时间注意力权重
        weighted_x = x * temporal_weights
