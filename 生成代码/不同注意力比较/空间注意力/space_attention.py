...
class SpatialAttention(Layer):
    ...
    def build(self, input_shape):
        # 创建空间注意力权重参数
        self.spatial_W = self.add_weight(shape=(input_shape[-3], input_shape[-2], 1), initializer='random_normal', trainable=True)
        self.spatial_b = self.add_weight(shape=(input_shape[-3], input_shape[-2]), initializer='zeros', trainable=True)
        super(SpatialAttention, self).build(input_shape)

    def call(self, x):
        # 计算空间注意力权重
        spatial_weights = K.softmax(K.dot(x, self.spatial_W) + self.spatial_b, axis=[-3, -2])

        # 应用空间注意力权重
        spatial_weighted_x = x * spatial_weights

        return spatial_weighted_x
