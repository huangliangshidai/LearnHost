...
from tensorflow.keras.layers import Input,GlobalAveragePooling2D, Dense, Reshape, Multiply, Lambda, Add, Concatenate
...
class CBAM(Layer):
    ...
    def build(self, input_shape):
        ...
        # 通道注意力
        self.channels_avg = GlobalAveragePooling2D()(self)
        self.channels_max = GlobalMaxPooling2D()(self)
        self.channels_avg = Reshape((1, 1, channels))(self.channels_avg)
        self.channels_max = Reshape((1, 1, channels))(self.channels_max)
        self.channels_shared = Dense(channels // self.ratio, activation='relu')
        self.channels_shared = self.channels_shared(self.channels_avg + self.channels_max)
        self.channels_weight = Dense(channels, activation='sigmoid')
        self.channels_weight = self.channels_weight(self.channels_shared)
        self.channels_weight = Reshape((1, 1, channels))(self.channels_weight)
        # 空间注意力
        self.spatial_avg = GlobalAveragePooling2D()(self)
        self.spatial_max = GlobalMaxPooling2D()(self)
        self.spatial_feature = Concatenate()([self.spatial_avg, self.spatial_max])
        self.spatial_shared = Dense(1, activation='relu')
        self.spatial_weight = Dense(2, activation='softmax')
        self.spatial_shared = self.spatial_shared(self.spatial_feature)
        self.spatial_weight = self.spatial_weight(self.spatial_shared)
        self.spatial_weight = Reshape((1, 1, 2))(self.spatial_weight)

    def call(self, inputs):
        # 通道注意力部分
        channels_avg = self.channels_avg
        channels_max = self.channels_max
        channels_shared = self.channels_shared
        channels_weight = self.channels_weight
        channels_weighted_feature = Multiply()([inputs, channels_weight])
        # 空间注意力部分
        spatial_avg = self.spatial_avg
        spatial_max = self.spatial_max
        spatial_shared = self.spatial_shared
        spatial_weight = self.spatial_weight
        spatial_weighted_feature = Multiply()([channels_weighted_feature, spatial_weight])
        # 合并通道和空间注意力
        cbam_feature = Add()([channels_weighted_feature, spatial_weighted_feature])
        return cbam_feature
