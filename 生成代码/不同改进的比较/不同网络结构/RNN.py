...
from tensorflow.keras.layers import SimpleRNN, Input, Dense,concatenate

# 创建一个全零的 NumPy 数组作为 SimpleRNN 的初始状态
# initial_state = Input(shape=(128, ))
# 输入层
input_layer = Input(shape=(X_train.shape[1], X_train.shape[2]))
# SimpleRNN层

# rnn_layer = SimpleRNN(128, return_sequences=True, return_state=False)(input_layer, initial_state=initial_state)
rnn_layer = SimpleRNN(128, return_sequences=True, return_state=False)(input_layer)
# 在SimpleRNN之后添加Flatten层
x = Flatten()(rnn_layer)  # 如果你的SimpleRNN层被命名为rnn_layer
...
# 输出层
output_layer = Dense(10, activation='softmax')(dropout_layer)  # 注意这里将 rnn_layer 替换为 dense_layer
...
assert X_train.shape[1:] == (128, 862), "X_train shape does not match model input shape."
assert X_test.shape[1:] == (128, 862), "X_test shape does not match model input shape."
