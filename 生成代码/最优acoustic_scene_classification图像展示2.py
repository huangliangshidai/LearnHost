...
def load_data(dataset_path, num_classes):
    ...
    for folder_name in os.listdir(dataset_path):
        ...
        for filename in os.listdir(folder_path):
            ...
            try:
                # 数据增强
                for _ in range(2):  # 每个原始样本生成2个增强样本
                    audio_data, sample_rate = librosa.load(file_path, sr=None)
                    augmented_data = data_augmentation(audio_data, sample_rate)
                    # 使用librosa库加载音频文件，并提取其特征
                    feature = extract_feature(augmented_data, duration=len(augmented_data) / sample_rate)
                    X.append(feature)
                    y.append(label)
...
def extract_feature(file_path, duration=None):
    # 提取音频特征
...
# 保存模型
model = load_model('trained_model_l2.h5')
print("模型已保存")